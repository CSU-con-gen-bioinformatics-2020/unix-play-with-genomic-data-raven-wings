...
```
10! (not including writing 'ls'; 13 with the ls )
```
...
```
To make sure you are calling up the right file and file name so you don't accidentally delete the wrong thing.
```

# If you had just typed `rm -r bad directory name with spaces` what would Unix have tried to remove?

```
It would have tried to remove 5 directories with the names bad, directory, name, with, and spaces.
```
...
```{bash}
mkdir A05_S5
cp bam/*A05_S5.bam sam/*A05_S5.sam fastq/*A05_S5*.fq.gz A05_S5
ls A05_S5
```
...
```
They are the same except for /1 and /2 at the end. Seems to be some sort of a code for the file name.
```
